...
| `CampaignEconomy` | Per-day warband economy (income/wages/upkeep, morale drift, recruit regen, desertion); `CampaignState.ApplyDayTick` delegates to it. |
...
| `ArenaAtmosphere` | Battle weather (rain/snow/mist) particles + ambience, sky dressing, and ambient air particles; built by `BattleBootstrap`. |
